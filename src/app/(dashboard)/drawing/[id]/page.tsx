...
import { getDrawingByIdAction, updateDrawingAction } from "@/actions/drawing/drawing-action";
...
const DrawingWorkspace = () => {
  ...
  const getDrawingData = useCallback(async () => {
    try {
      const { data, success } = await getDrawingByIdAction(id);
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {isDirty && <span className="text-xs text-amber-500 animate-pulse">有未保存的更改</span>}
          <Button
